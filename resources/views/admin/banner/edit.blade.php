...
@section('main')
<form action="{{route('banner.update',['id'=>$model->id])}}" method="POST" role="form">
	@csrf
	...
	<div class="form-group">
		<label for="">Tên</label>
		<input type="text" class="form-control" name="name" id="" value="{{$model->name}}" placeholder="">
		@if($errors->has('name'))
		{{$errors->first('name')}}
		@endif
	</div>
	...
	<div class="form-group">
		...
		<div class="input-group">
			<input type="text" class="form-control" name="image " value="{{$model->image}}" id="image" placeholder="">
			@if($errors->has('image'))
			{{$errors->first('image')}}
			@endif
		...
		<img src="{{$model->image}}" alt="" id="show_img"  style="width: 30%">
	</div>
